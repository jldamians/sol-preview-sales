...
async function _generateSalesPreview() {
  try {
    ...
    await clear(this.workspace, 'input[name="periodoRegistroVentasConMovim"]');
...
async function _getSalesInformation() {
  try {
    ...
    throw new Error(
      `El preliminar del registro de ventas no está disponible (${exception.message})`
    )
  }
}
...
function _getFormattedOutputData() {
  return this.data.filter((cpe) => {
    ...
    information.customer = {
      // extraemos el último caracter de la cadena debido a que el valor puede tener más de 1 caracter,
      // y el tipo de documentos tiene solo un caracter (por ejemplo 1 o 6)
      identityType: customerIdentityType.substr(-1),
